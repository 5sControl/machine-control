...
from machine_control_utils.utils import send_report_and_save_photo, calculate_intersection, get_areas
...
def main(model: Model, img, area_values):
    ...
    for i, a_val in enumerate(area_values):
        ...
        cv2.rectangle(img, huge_box, (0, 0, 200), 2) # red area
        cv2.putText(img, str(a_val.zone_name) + " : " + str(a_val.zone_id), (huge_box[0], huge_box[1] - 10),
        ...
        for track in tracks:
            ...
            small_box = x1, y1, x2 - x1, y2 - y1
            cv2.rectangle(img, small_box, (255, 0, 0), 2) # blue
            if calculate_intersection(img, small_box, huge_box) > model.person_in_frame:
                in_area = True
            ...
            if in_area:
                cv2.rectangle(img, huge_box, (0, 200, 0), 2) # green area
            else:
